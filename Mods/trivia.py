...
class triviaGame:
    ...
    def getLocalLeaderboard(self, serverID, connection):
        localScores = []
        with connection.cursor() as cursor:
            cursor.execute(utils.concat(("SELECT tblUser.userID, tblUser.intScore FROM tblUser, tblServerUser WHERE tblServerPrefixes.serverID = ",serverID," ORDER BY tblUser.intScore DESC;")))
            for row in cursor:
